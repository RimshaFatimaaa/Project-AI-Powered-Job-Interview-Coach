...
class LLMProcessor:
    ...
    def _init_prompt_templates(self):
        ...
        self.question_generation_template = PromptTemplate(
            input_variables=["question_type", "topic", "role", "skill", "difficulty", "context"],
            template="""You are an expert HR interviewer. Generate a {difficulty} level {question_type} question about {topic} for a {role} position.
...
Question:"""
        ...
        self.evaluation_template = PromptTemplate(
            input_variables=["question", "answer", "criteria", "expected_keywords"],
            template="""You are an expert interviewer evaluating a candidate's response. Analyze the following:
...
Evaluation:"""
    ...
    def _generate_with_local(self, template: str, topic: str, role: str, skill: str, difficulty: str, context: str) -> str:
        """Generate question using local model"""
        prompt = template.format(
            topic=topic, role=role, skill=skill, difficulty=difficulty
        )
    ...
    def _evaluate_with_local(self, question: str, answer: str, criteria: str, keywords: str) -> str:
        """Evaluate answer using local model"""
        prompt = self.evaluation_template.format(
            question=question,
            answer=answer,
            criteria=criteria,
        ...
        generated_text = result[0]['generated_text']
        evaluation = generated_text[len(prompt):].strip()
